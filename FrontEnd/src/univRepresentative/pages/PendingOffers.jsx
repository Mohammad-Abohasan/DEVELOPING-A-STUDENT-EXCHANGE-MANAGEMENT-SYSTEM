import {
  ...
  const columns = [
    ...
    },
    {
      ...
      headerName: "Details",
      renderCell: (params) => (
        <Link to={`/universityRepresentative/pendingOffers/${params.id}`} className="btn btn-bg" style={{ width: "100%", backgroundColor: "#146eb4", color: "#FFFFFF" }}>
          Details
        </Link>
      ),
      sortable: false,
  ...
  const PendingOffers = () => {
    ...
    const getPendingOffers = async () => {
      try {
        const response = await axios.get('/offer/pendingOffers', {
          headers: {
    ...
    return (
      <>
        {offersData.length === 0 ? (
          ...
          <>
            <h2 className="pt-4 pb-2 px-3">Pending Offers</h2>
